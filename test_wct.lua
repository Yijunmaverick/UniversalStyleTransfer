...
function feature_swap_whiten(cF, sF)
   ...
   --local adabn = nn.InstanceNormalization(cF:size(1), false)
   ...
   -- content feature whitening
    ...
    --local s_d = torch.cdiv(torch.ones(sz[1]-k+1), torch.sqrt(s_e[{{k,sz[1]}}])) 
    local s_d = torch.sqrt(s_e[{{k,sz[1]}}]):pow(-1)
    local n_styleFeature = s_v[{{},{k,sz[1]}}]*torch.diag(s_d)*(s_v[{{},{k,sz[1]}}]:t())*styleFeature1
    ...
    local n_contentFeature = c_v[{{},{k,sg[1]}}]*torch.diag(c_d)*(c_v[{{},{k,sg[1]}}]:t())*contentFeature1
...
local function styleTransfer(content, style)
    ...
    --print('WCT4 Time: ' .. timer:time().real .. ' seconds')
    sF4 = nil
    ...
    --print('WCT3 Time: ' .. timer:time().real .. ' seconds')
    sF3 = nil
    ...
    --print('WCT2 Time: ' .. timer:time().real .. ' seconds')
    sF2 = nil
    ...
    --print('WCT1 Time: ' .. timer:time().real .. ' seconds')
    sF1 = nil
