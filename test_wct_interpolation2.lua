...
cmd:option('-interpolation_weight', 0.5)
...
function feature_wct(cF, sF1, sF2)
    ...
    local tFeature = opt.interpolation_weight * tFeature1 + (1 - opt.interpolation_weight) * tFeature2 
...
for i=1,numContent do
    ...
    if opt.synthesis == 0 then
        ...
        local savePath = paths.concat(opt.outputDir, contentName .. '_stylized_interpolation_' ..'weight_' ..opt.interpolation_weight*100 .. '.' .. opt.saveExt)
